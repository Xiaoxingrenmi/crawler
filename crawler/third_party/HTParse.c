...
PUBLIC char* HTParse(const char* aName, const char* relatedName, int wanted) {
  ...
  if ((result = (char*)HT_MALLOC(len)) ==
      NULL) /* Lots of space: more than enough */
...
PUBLIC char* HTRelative(const char* aName, const char* relatedName) {
  ...
  if (!after_access) { /* Different access */
    ...
    for (; *q && *q != '#' && *q != ';' && *q != '?'; q++)
      if (*q == '/')
        levels++;
    if ((result = (char*)HT_MALLOC(3 * levels + strlen(last_slash) + 4)) ==
        NULL)
